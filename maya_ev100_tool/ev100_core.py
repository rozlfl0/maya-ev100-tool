...
@dataclass(frozen=True)
class PixelEVCalibrationResult:
    """Exposure-stop correction from Arnold RenderView pixel EV on a 0.18 gray card."""

    pixel_ev: float
    correction_stops: float
    current_exposure: float = 0.0
    recommended_exposure: float = 0.0
...
def estimate_exposure_calibration_from_pixel_ev(pixel_ev: float, current_exposure: float = 0.0) -> PixelEVCalibrationResult:
    """Estimate exposure correction from Arnold RenderView Pixel Inspector EV.

    On a 0.18 middle-gray card, Arnold's pixel EV should be near 0. Positive
    pixel EV means the gray card rendered too bright, so the light exposure must
    be reduced by the same number of stops.
    """
    pixel_ev = float(pixel_ev)
    current_exposure = float(current_exposure)
    correction_stops = -pixel_ev
    return PixelEVCalibrationResult(
        pixel_ev=pixel_ev,
        correction_stops=correction_stops,
        current_exposure=current_exposure,
        recommended_exposure=current_exposure + correction_stops,
    )
